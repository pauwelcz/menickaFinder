...
async function suziesCheerioParser({url, name}, date) {
    ...
    $(`div.day[data-day="${date.dayInWeek}"] > div`).each(function (_index, element) {
        menu += printModules.addItemToMenu({
    ...
    menu += $(`div.day[data-day="${date.dayInWeek}"] > p`).text().trim()
    menu = menu.trim(); // v případě, že by neměli v nabídce kopečkovou zmrzlinu :)
