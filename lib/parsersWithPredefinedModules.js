...
async function menickaParser({urlMenicka, name}, date) {
    ...
    */
    ...
    $('div.menicka').each(function (_index, element) {
        if ($(element).find("div.nadpis").text().indexOf(`${date.day}.${date.month}.${date.year}`) !== -1) {
            ...
            menu$('li').each(function (_index, element) {
                ...
                menu += printModules.addItemToMenu({
                    ...
                    price: `${($(element).find("div.cena").text().trim().length) ? $(element).find("div.cena").text().trim() : "0 Kč"}`.replaceAll(" Kč", ",- Kč"), 
                });
            });
        ...
        if ($(element).find("h4").text().trim().indexOf(`${date.day}.${date.month}.`) !== -1) {
            ...
            menu$('div.item').each(function (_index, element) {
                menu += printModules.addItemToMenu({
                    ...
                    price: `${($(element).find("div.price").text().length) ? $(element).find("div.price").text().trim()  : "0"},- Kč`,
                });  
            });
