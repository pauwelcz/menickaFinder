...
async function menickaParser({urlMenicka, name}, date) {
    if (urlMenicka == undefined) return {
        name,
        error: "BADURLKEY"
    }
    // čitelnější zobrazení
    const dictionary = {
        polevka: "Polévka",
        jidlo: "Jídlo"
    }
    ...
    */
    ...
    $('div.menicka').each(function (_index, element) {
        if ($(element).find("div.nadpis").text().trim().indexOf(`${date.day}.${date.month}.${date.year}`) !== -1) {
            const menu$ = cheerio.load($(element).html(), {decodeEntities: false})
            menu$('ul > li').each(function (_index, element) {
                /**
                 ...
                 *   </div>
                 */
                if ($(element).find("div.polozka").text().trim().length == 0) {
                    menu += $(element).text();
                    return false
                }
                 ...
                 */
                const alergens = [];
                $(element).find("div.polozka > em").each(function (_index, element) {
                    alergens.push(element.attribs.title);
                });

                menu += printModules.addItemToMenu({
                    category: dictionary[element.attribs.class],
                    title: $(element).find("div.polozka").text().trim(),
                    description: `Možné alergeny: ${alergens.join(', ')}`,
                    price: `${($(element).find("div.cena").text().trim().length !== 0) ? $(element).find("div.cena").text().trim() : "0 Kč"}`.replaceAll(" Kč", ",- Kč"), 
                });
            });
...
async function suziesCheerioParser({url, name}, date) {
    ...
    $('div#weekly-menu > div.day').each(function (_index, element) {
        if ($(element).find("h4").text().trim().indexOf(`${date.day}.${date.month}.`) !== -1) {
            const menu$ = cheerio.load($(element).html(), {decodeEntities: false})
            menu$('div.item').each(function (_index, element) {
                menu += printModules.addItemToMenu({
                    category: $(element).find("h6").text().trim(),
                    title: $(element).find("div.title").text().trim(),
                    description: $(element).find("div.text").text().replace(/\n\s+/g, ' ').trim(),
                    price: `${($(element).find("div.price").text().trim().length !== 0) ? $(element).find("div.price").text().trim()  : "0"},- Kč`,
                });  
            });
 ...
 */
 async function uCapaCheriioParser({url, name}, date) {
    ...
    $(`div.listek > div.row`).each(function (_index, element) {
        ...
        if ($(element).find("div.date").text().trim() == `${date.day}\. ${date.month}\. ${date.year}`) {
            ...
            // vlozim zbytek jidel
            const contentForDay = cheerio.load($(element).html(), { decodeEntities: false });
            contentForDay("div.row").each(function (_index, element) {
                if ($(element).find("div.food").text().trim().length != 0) {
                    menu += printModules.addItemToMenu({
                        category: "Jídlo",
                        title: $(element).find("div.food").text().trim(),
                        description: "",
                        price: $(element).find("div.price").text().trim().replace(" K", ",- K"),
                    });
                }
            });
